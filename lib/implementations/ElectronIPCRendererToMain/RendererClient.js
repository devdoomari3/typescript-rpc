"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const defer_promise_1 = __importDefault(require("defer-promise"));
const electron_1 = require("electron");
const v4_1 = __importDefault(require("uuid/v4"));
const BaseAPIClient_1 = require("../../BaseAPIClient");
const common_1 = require("./common");
class RendererIPCClient extends BaseAPIClient_1.BaseAPIClient {
    constructor() {
        super(...arguments);
        this.responseHandlers = {};
    }
    __callAPI(api) {
        return (req) => __awaiter(this, void 0, void 0, function* () {
            const requestId = v4_1.default();
            const futureResponse = defer_promise_1.default();
            this.responseHandlers[requestId] = futureResponse;
            const reqToSend = {
                funcArgs: req,
                funcName: api.name,
                requestId,
            };
            electron_1.ipcRenderer.send(common_1.channelName, reqToSend);
            return futureResponse.promise;
        });
    }
}
exports.RendererIPCClient = RendererIPCClient;
